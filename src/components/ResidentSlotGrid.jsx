...
function ResidentSlotCard({ slot, isMySlot, onFree, index }) {
  ...
  return (
    ...
    >
      ...
      {/* Actions — only Free Slot if this resident owns it */}
      {isMySlot && (
        <div className="border-t border-parking-border pt-3">
          <p className="text-xs font-mono text-parking-green mb-2">✅ Your slot</p>
          <button
      ...
      {slot.isOccupied && !isMySlot && (
        <div className="border-t border-parking-border pt-3">
          <p className="text-xs font-mono text-parking-muted">Occupied by someone else</p>
        </div>
      )}
    </div>
  )
...
export default function ResidentSlotGrid({ slots, mySlots, onFree }) {
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 glass border border-parking-border rounded-3xl">
          ...
          <p className="text-parking-muted text-sm mt-1">
            {slots.length === 0 ? 'No parking slots available' : 'Try a different filter'}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
        {filtered.map((slot, i) => (
          <ResidentSlotCard
            ...
            slot={slot}
            index={i}
            isMySlot={mySlots.some(s => s.id === slot.id)}
            onFree={onFree}
          />
        ))}
      </div>
